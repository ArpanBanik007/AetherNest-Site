...
import { 
  ...
  Filter, MoreVertical, Download, ArrowUpRight 
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16">
          <div>
            ...
            <p className="text-rich-dark/40 font-medium text-sm">System status: <span className="text-emerald-500 font-bold uppercase tracking-widest text-[10px]">Optimal</span>. 12 pending property audits.</p>
          </div>
          <div className="flex items-center gap-4">
            <button className="px-8 py-4 rounded-2xl bg-white border border-gray-100 text-[10px] font-bold text-rich-dark uppercase tracking-[0.2em] flex items-center gap-3 hover:bg-gray-50 transition-all shadow-sm">
              <Download size={16} className="text-rich-dark/20" /> Export Analytics
            </button>
            <GlowButton variant="emerald" className="px-10 py-4 text-[10px]">
              Add New Asset
            </GlowButton>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-16 mb-16">
          ...
          <div className="bg-white p-12 rounded-[4rem] border border-gray-100 shadow-premium relative overflow-hidden">
            ...
            <div className="space-y-8">
              {[1, 2, 3].map((item) => (
                <div key={item} className="flex items-center gap-6 group cursor-pointer">
                  <div className="w-16 h-16 rounded-[1.5rem] bg-gray-50 overflow-hidden border border-gray-100 flex-shrink-0 group-hover:scale-105 transition-transform duration-500">
                    <img src={`https://images.unsplash.com/photo-1600585154340-be6161a56a0c?q=80&w=100&auto=format&fit=crop`} className="w-full h-full object-cover" alt="Property" />
                  </div>
                  <div className="flex-1">
                    <h5 className="text-sm font-extrabold text-rich-dark mb-1">Palm Villa {item}04</h5>
              ...
              <GlowButton variant="emerald" className="w-full py-5 text-[10px] mt-6">
                Enter Audit Center
              </GlowButton>
            </div>
          </div>
          ...
          <div className="p-12 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              <div className="relative flex-1 md:w-80">
                ...
                <input type="text" placeholder="Search by PID, Agent, or Location..." className="w-full bg-gray-50 border-none rounded-2xl py-4 pl-16 pr-6 text-xs font-bold text-rich-dark focus:bg-white focus:ring-2 focus:ring-primary/10 transition-all outline-none" />
              </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50">
                  ...
                  <th className="p-10 text-[10px] font-bold text-rich-dark/20 uppercase tracking-[0.3em]">Senior Lead</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {[
                  { name: 'The Glass Pavilion', agent: 'Marcus Valerius', status: 'Active', price: '$12.5M' },
                  { name: 'Azure Marina Suite', agent: 'Elena Rodriguez', status: 'Sold', price: '$4.2M' },
                  { name: 'Sky Garden Villa', agent: 'David Cohen', status: 'Pending', price: '$8.9M' }
                ].map((row, i) => (
                  <tr key={i} className="group hover:bg-gray-50/30 transition-all cursor-default">
                    <td className="p-10">
                      <div className="flex items-center gap-6">
                        ...
                        <span className="text-md font-extrabold text-rich-dark">{row.name}</span>
                      </div>
                    </td>
                    <td className="p-10">
                      <span className="text-[10px] font-bold text-rich-dark/40 uppercase tracking-[0.1em]">{row.agent}</span>
                    </td>
                    <td className="p-10">
                      <div className="flex items-center gap-3">
                        {row.status === 'Active' ? <CheckCircle size={16} className="text-emerald-500" /> : row.status === 'Sold' ? <Building2 size={16} className="text-primary" /> : <Clock size={16} className="text-amber-500" />}
                        <span className={`text-[9px] font-bold uppercase tracking-[0.25em] ${row.status === 'Active' ? 'text-emerald-500' : row.status === 'Sold' ? 'text-primary' : 'text-amber-500'}`}>{row.status}</span>
                      </div>
                    </td>
                    <td className="p-10">
                      <span className="text-lg font-extrabold text-rich-dark">{row.price}</span>
                    </td>
                    <td className="p-10">
                      <button className="p-3 text-rich-dark/10 hover:text-primary transition-colors rounded-xl hover:bg-primary/5">
